...
<head>
    <meta charset="UTF-8">
    <title>Academia - Acesso Alunos</title>
    ...
    <main class="content">
        ...
        <section class="card">
            ...
            <div style="display:flex; gap:10px; justify-content:space-between; align-items:center; margin-top:12px;">
                <form method="GET" action="{{ route('academia.acesso_alunos') }}" style="display:flex; gap:8px;">
                ...
                <a href="{{ route('academia.acesso_alunos.create') }}" class="btn edit">+ Criar Acesso</a>
            </div>
                ...
                <tbody>
                    @forelse ($alunos as $aluno)
                        <tr>
                            ...
                            <td>{{ data_get($aluno, 'email_encarregado', '-') }}</td>
                            <td>{{ data_get($aluno, 'acesso', '-') }}</td>
                            <td>{{ data_get($aluno, 'created_at', '-') }}</td>
                            <td style="display:flex; gap:6px;">
                                <a href="{{ route('academia.acesso_alunos.edit', data_get($aluno, 'id')) }}" class="btn edit">Editar</a>
                                <form method="POST" action="{{ route('academia.acesso_alunos.destroy', data_get($aluno, 'id')) }}" onsubmit="return confirm('Deseja deletar este acesso?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" style="text-align:center;">Sem alunos cadastrados.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
